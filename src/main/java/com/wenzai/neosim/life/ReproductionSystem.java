...
public class ReproductionSystem
{
    ...
    // 临产目标缓存，分娩后清除
    private static final Map<UUID, BlockPos> BIRTH_TARGETS = new HashMap<>();
    ...
    private static void tryPregnant(ServerLevel level, String city, Entity mother)
    {
        ...
        LifeSystem.announce(level, city,
                "§f好消息！§f" + mother.getPartner() + " §e和 §f" + mother.getNpcName() + " §e要有宝宝了！");
    ...
    {
        UUID id = mother.getUUID();
        BlockPos target = BIRTH_TARGETS.get(id);
        if (target == null)
        {
    ...
    private static void goToClinic(ServerLevel level, String city, Entity mother)
    {
        ...
        if (clinic == null)
        {
            // 无诊所：目标记为居家分娩
            BlockPos home = mother.getHomePos();
            BIRTH_TARGETS.put(mother.getUUID(), home != null ? home : mother.blockPosition());
            return;
        }
        BIRTH_TARGETS.put(mother.getUUID(), clinic);
    ...
    private static void giveBirth(ServerLevel level, String city, Entity mother, BlockPos birthPos)
    {
        ...
        // 公告+出生音效
        LifeSystem.announce(level, city, "§f" + baby.getNpcName() + " §e诞生了！");
    ...
    private static List<Entity> loadedNpcs(ServerLevel level, String city)
    {
        ...
        {
            if (e instanceof Entity npc && city.equals(npc.getCityName()))
            {
                list.add(npc);
            }
        }
