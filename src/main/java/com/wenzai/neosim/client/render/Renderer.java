...
public class Renderer extends HumanoidMobRenderer<Entity, Model<Entity>>
{
	...
	// 文件皮肤纹理缓存（M11：LRU 上限 64，超限释放最久未用纹理，防跨存档无界累积）
	private static final int FILE_TEXTURE_CACHE_MAX = 64;

	private static final Map<String, ResourceLocation> FILE_TEXTURE_CACHE =
			new java.util.LinkedHashMap<>(FILE_TEXTURE_CACHE_MAX, 0.75f, true)
			{
				@Override
				protected boolean removeEldestEntry(Map.Entry<String, ResourceLocation> eldest)
				{
					if (size() > FILE_TEXTURE_CACHE_MAX)
					{
						// 从纹理管理器注销并释放 GPU 纹理，防显存泄漏
